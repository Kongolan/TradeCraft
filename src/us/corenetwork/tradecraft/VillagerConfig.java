...
public class VillagerConfig {
	...
	public static List<CustomRecipe> getTrades(String career, int tier) {
		...
		List<?> tradesConfig = (List<?>) IO.config.get("Tiers." + career + "."
				+ tier);
		...
		for (Object mapO : tradesConfig) {
			...
			ItemStack itemA = getItemStack((Map<?, ?>) map.get("itemA"), null,
					null);
			...
			CustomRecipe recipe;
			if (itemB == null)
				recipe = new CustomRecipe(CraftItemStack.asNMSCopy(itemA),
						CraftItemStack.asNMSCopy(itemC));
			else {
				itemB.setAmount(Math.min(itemB.getAmount(), 64));
				recipe = new CustomRecipe(CraftItemStack.asNMSCopy(itemA),
						CraftItemStack.asNMSCopy(itemB),
						CraftItemStack.asNMSCopy(itemC));
	...
	private static ItemStack getItemStack(Map<?, ?> map, ItemStack itemA,
			ItemStack itemB) {
		int id;
		int amount = 0;
		Integer data = 0;
		...
		if (map.containsKey("currency")) {
			...
			if (idFromName == null) {
				Logs.warning("Invalid trades config: Unknown currency item name:"
						+ currencyName + "!");
				return null;
			}
			id = idFromName;
		...
		if (map.containsKey("data")) {
			data = getRandomNumber(map.get("data"));
		}
		...
		if (yamlNbtTag != null) {
			...
			if (yamlNbtTag instanceof String) {
				...
				if (newTag == null) {
					Logs.warning("Invalid config! Nanobot file "
							+ ((String) yamlNbtTag) + ".yml is missing!");
				}
			} else {
		...
		List<Map<String, ?>> enchants = (List<Map<String, ?>>) map
				.get("enchants");
		if (enchants != null) {
	...
	private static int getDamageValue(int itemID, Object value) {
		if (value == null)
			...
			if (itemID == Material.WOOL.getId()
					|| itemID == Material.INK_SACK.getId()
					|| itemID == Material.STAINED_GLASS.getId()
					|| itemID == Material.STAINED_GLASS_PANE.getId()
					|| itemID == Material.CARPET.getId()) {
				DyeColor color = (DyeColor) Util.findEnum(DyeColor.values(),
						(String) value);
				if (color == null) {
				...
				if (itemID == Material.INK_SACK.getId())
					return color.getDyeData(); // Dyes have different color
												// values
	...
	public static String getRandomCareer(int villagerType) {
		...
		List<String> careers = IO.config.getStringList("Professions."
				+ villagerType);
		if (careers.size() == 0) {
			if (!Settings
					.getBoolean(Setting.KEEP_VANILLA_IF_NO_TRADES_SPECIFIED))
				Logs.severe("No professions found for villager number "
						+ villagerType + ".");
			return null;
		}
